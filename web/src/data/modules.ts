...
export async function fetchModules(): Promise<Module[]> {
    ...
    return json.data.map((mod: any) => {
        ...
        let amount = 0;
        if (mod.attributes) {
            const firstKey = Object.keys(mod.attributes).find((k) => typeof mod.attributes[k] === "number");
            if (firstKey) {
                unit = firstKey;
        ...
        return {
            ...
            width: mod.attributes?.Space_X || dims.width,
            depth: mod.attributes?.Space_Y || dims.depth,
            height: dims.height,
            power: mod.attributes?.Usable_Power || 0,
            weight: mod.attributes?.Weight || 0,
