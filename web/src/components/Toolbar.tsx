...
export default function Toolbar() {
    ...
    return (
        <aside className="absolute left-0 top-45 ml-6 mt-6 z-20 flex flex-col items-center gap-2 py-8 h-[480px] bg-card/80 rounded-r-xl shadow-lg border-r border-border">
            <TooltipProvider>
                <ToggleGroup type="single" value={selected} onValueChange={(v) => setSelected(v || "hand")} className="flex flex-col gap-2 h-full justify-center">
                    {TOOLS.map((tool) => (
                        <Tooltip key={tool.value}>
                            <TooltipTrigger asChild>
                                <ToggleGroupItem
                                    ...
                                    className={`w-10 h-10 flex items-center justify-center rounded-md ${selected === tool.value ? "bg-primary text-primary-foreground" : ""}`}
                                >
